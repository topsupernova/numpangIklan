...
import { StyleSheet, View, Text, Image, ListView, Keyboard, TouchableHighlight, ActivityIndicator, Alert } from 'react-native';

import Icons from '../api/FontAwesomeIcons';
import fetchIklans from '../api/apicatlist';

export default class Rowkatlist extends Component {
  constructor(props) {
    super(props);
    const idx = this.props.rowCat;
    //const rmod = this.props.rowMod;
    //Alert.alert("Alert "+idx);
  ...
  _getNextPage() {
    ...
    fetchIklans(self.state.idx, self.state.pageSize)
      .then(function(mycat) {
        const data = self.state.data.concat(mycat);
